/* Funcoes auxiliares para uma calculadora avançada */
...
// Essas variaveis vem do Flex/Bison
extern FILE *yyin;
extern FILE *yyout;
...
void treefree(struct ast *a) {
    switch (a->nodetype) {
        ...
        case 'I': case 'W': case 'R':
            ...
            if (((struct flow *)a)->il) treefree(((struct flow *)a)->il);
            break;

        default:
            fprintf(yyout, "erro interno: free bad node %c\n", a->nodetype);
    }
...
double eval(struct ast *a) {
    ...
    switch (a->nodetype) {
        ...
        /* for */
        case 'R':            
            eval(((struct flow*)a)->el); /* avalia o INIT*/

            if(((struct flow *)a)->tl){ /* testa se lista de comandos nao eh vazia*/
                v = 0.0; /* valor default */
                while(eval(((struct flow *)a)->cond) != 0){ /* avalia a condicao */
                    v = eval(((struct flow *)a)->tl); /* avalia comandos*/
                    eval(((struct flow*)a)->il); /* avalia o INC */
                }
            }
            break; /* valor do ultimo comando eh valor do while/do */
        ...
        case 'C':
            ...
            break;
        default:
            yyerror("erro interno: bad node %c\n", a->nodetype);
    }
    return v;
}
...
static double callbuiltin(struct fncall *f) {
    ...
    switch (functype) {
        ...
        case B_print:
            ...
            return v;
        default:
            yyerror("Função predefinida %d desconhecida\n", functype);
            return 0.0;
    }
}
...
static double calluser(struct ufncall *f) {
    ...
    // Preparar espaço para salvar argumentos
...
int main(int argc, char **argv) {
    // Verificando se foi passado o parametro
    if (argc < 2) {
        printf("Nao foi informado o caminho do arquivo de entrada.\n");
        return 1;
    }
    ...
    printf("Os resultados podem ser conferidos no arquivo \"saida.txt\"\n");
